...
app.whenReady().then(() => {
  ...
  ipcMain.handle('appearance', () => ({ platform: process.platform, theme: nativeTheme.themeSource, accent: ['darwin', 'win32'].includes(process.platform) ? systemPreferences.getAccentColor?.() || null : null }));
